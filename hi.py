...
class IQDataGenerator():
    ...
    def __getitem__(self, index: int) -> Tuple[cp.ndarray, cp.ndarray]:
        B = self.batch_size; tx_len = self.tx_hop_signal_len; slots_per_batch = B // self.CNN_inputs_per_slot; max_jammed_hops_per_slot = cp.ceil(self.num_hops*self.maximum_jamming_pecentage)
        rampupBitsLen = int(self.rampupSymLen * cp.log2(self.modulationOrderOther)); rampdownBitsLen = int(self.rampdownSymLen * cp.log2(self.modulationOrderOther))
        X = cp.zeros((B, self.CNN_input_size, 3), dtype=cp.float32); jammed_idx_lis = cp.zeros((slots_per_batch,self.num_hops, tx_len), dtype=cp.int8)
        batch_seed = None if self.seed is None else int(self.seed + index); rng = cp.random.RandomState(batch_seed)
        rbits_up = self.rng.randint(0, 2, (1,rampupBitsLen), dtype=cp.int8); rbits_dn = self.rng.randint(0, 2, (1,rampdownBitsLen), dtype=cp.int8)
        rampupsym = qpsk_mod(rbits_up,self.QPSK_gain).astype(cp.complex64)[0,:]; rampdownsym = qpsk_mod(rbits_dn,self.QPSK_gain).astype(cp.complex64)[0,:]
        snr = rng.choice(self.snr_array,size=slots_per_batch); snr = cp.broadcast_to(snr[:, None, None], (slots_per_batch, self.num_hops, 1)); cfo = cp.array(260); cfo = cp.full((slots_per_batch, self.num_hops, 1), cfo)
        jammed_hops_per_slot = rng.choice(cp.arange(max_jammed_hops_per_slot+1),slots_per_batch)
        perms = cp.argsort(cp.random.rand(slots_per_batch, self.num_hops), axis=1); mask = perms < jammed_hops_per_slot[:, None]; jammed_hops_vec_loc= cp.where(mask, 1, 0)
        tx_bits = rng.randint(0, 2,(slots_per_batch, self.num_hops*self.enBitsPerHop), dtype=cp.int8); modData = qam16_mod(tx_bits,self.QAM_16_gain); modData = modData.reshape(slots_per_batch,self.num_hops,self.dataSymLen)
        mod_frame = add_preamble_guard_postamble(modData, self.preamble, self.guard, self.postaamble,rampupsym,rampdownsym); signal_ups = upsample_stride(mod_frame, self.sps); signal_tx_fl_rrc = hopwise_fftconvolve(signal_ups,self.h)
        signal_tx_fl_rrc = signal_tx_fl_rrc[:,:,(self.rrcFilterSpan*self.sps)//2:(self.rrcFilterSpan*self.sps)//2+self.tx_hop_signal_len]
        xw = (signal_tx_fl_rrc* self.ramp_window).astype(cp.complex64); signal_rx_noisy = awgn_measured(xw, snr, None)
        jamming_power_min = signalPowerLinear/(10**(self.JSRMindB/10)); jamming_power_max = signalPowerLinear/(10**(self.JSRMaxdB/10))
        signalPowerLinear = 10**(self.signalPowerdB/10); jamming_power = (jamming_power_min+ (jamming_power_max- jamming_power_min) * cp.random.rand(slots_per_batch, self.num_hops))*jammed_hops_vec_loc
        jamming_power = jamming_power[..., cp.newaxis]; jamming_signal_len = rng.randint(1, tx_len + 1,size=(slots_per_batch, self.num_hops))*jammed_hops_vec_loc
        jammed_sig = add_jamming(signal_rx_noisy, jamming_power, jamming_signal_len, jammed_idx_lis, rng); signal_rx_ch = frequency_offset(jammed_sig,cp.array(self.fs),cfo)
        signal_rx_ch = signal_rx_ch.reshape(slots_per_batch,self.num_hops*tx_len); jammed_idx_lis = jammed_idx_lis.reshape(slots_per_batch,self.num_hops*tx_len); adc_out = signal_rx_ch
        red_adc_out,red_jammed_idx_lis = reduce_input(adc_out,jammed_idx_lis, self.downsampling); scale = cp.ones((B,self.CNN_input_size))
        X[:,:,0] = cp.real(red_adc_out.reshape(B,self.CNN_input_size)); X[:,:,1] = cp.imag(red_adc_out.reshape(B,self.CNN_input_size)); X[:,:,2] = scale
        Y = red_jammed_idx_lis.reshape(B,self.CNN_input_size); return cp.asnumpy(X),cp.asnumpy(xw)
