...
def tx_rx_tone(freq, chan=0, fs=31.25e6, gain=0, rx_gain=30, buff_len=16384):
    ...
    # Set RX Gain (Manual gain is easier to verify reception)
    ...
    print('Starting simultaneous TX/RX. Press Ctrl+C to stop.')
    ...
    try:
        while not tx_stop_event.is_set():
            ...
            # Convert interleaved int16 samples back to complex float
            s_int16 = rx_buff[:2 * sr.ret]
            s_cplx = s_int16[0::2] + 1j * s_int16[1::2]
            s_cplx = s_cplx.astype(np.float32) / 32767.0 # Normalize

            # Calculate mean power (Magnitude squared)
            mean_power = np.mean(np.abs(s_cplx) ** 2)
            # Convert to dBFS (dB relative to Full Scale, where FS is 1.0 after normalization)
            power_dbfs = 10 * np.log10(mean_power + 1e-12) 

            # Display status (Using carriage return '\r' for live update)
...
def parse_command_line_arguments():
    ...
    parser.add_argument('-r', type=float, required=False, dest='rx_gain',
                        default=30.0, help='RX gain (New argument)')
...
if __name__ == '__main__':
    ...
    # Pass the new RX gain argument 'r' to the function
