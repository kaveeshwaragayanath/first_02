...
""" Transmits a tone out of the AIR-T and simultaneously receives data with AGC enabled, 
    then plots the final received spectrum. """
import sys
import numpy as np
import argparse
import SoapySDR
...
def plot_spectrum(s_cplx, freq, fs):
    """ Performs FFT and plots the received spectrum. """
    N = s_cplx.size
    ...
    # The division by N is for normalized amplitude scaling; square for power.
    ...
    plt.ylim(np.max(Pxx_dbfs) - 80, np.max(Pxx_dbfs) + 5) # Dynamic y-limit
...
def tx_rx_tone(freq, chan=0, fs=31.25e6, gain=0, rx_gain=30, buff_len=16384):
    ...
    try:
        ...
        if last_s_cplx.size > 0:
            print("Generating spectrum plot of the last received buffer...")
