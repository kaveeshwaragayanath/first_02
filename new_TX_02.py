...
def main(args):
    ...
    # Note: SOAPY_SDR_CF32 means Complex Float 32-bit (complex64 in NumPy/CuPy)
    # The TX stream expects COMPLEX samples, but your tx_sig is INT16.
    # If this is for a specific SDR (like AIR-T's high-speed stream), it might use int16.
    # We proceed with your int16 conversion, assuming the SDR backend handles it.
    ...
    # --- CORRECTED TX SIGNAL PREPARATION ---
    ...
    Y_reshaped = Y_complex_data.reshape(-1)
    
    # Ensure tx_sig_cplx is exactly pars.buff_len long (complex samples)
    if Y_reshaped.size < pars.buff_len:
        print(f"Warning: Generated signal size ({Y_reshaped.size}) < buffer length ({pars.buff_len}). Padding with zeros.")
        tx_sig_cplx = cp.pad(Y_reshaped, (0, pars.buff_len - Y_reshaped.size), mode='constant')
    else:
        tx_sig_cplx = Y_reshaped[0:pars.buff_len]

    tx_sig_cplx_np = cp.asnumpy(tx_sig_cplx)
    
    # Convert to interleaved int16 NumPy array (size 2 * pars.buff_len)
    tx_sig_int16 = make_tone(tx_sig_cplx_np) 
    
    # Submit the int16 buffer for transmission. 
    # Note: tx_buff_len is the number of complex samples (32768)
    tx_task = tx_executor.submit(tx_task_fn, sdr, tx_stream, tx_sig_int16, pars.buff_len, tx_stop_event)
    ...
    while not tx_task.done():
        try:
            sr = sdr.readStream(rx_stream, [buff], pars.buff_len) # Read complex64 data into CuPy buffer
            if sr.ret == SOAPY_SDR_OVERFLOW: # Data was dropped
                print('O', end='', flush=True)
                continue
            
            # --- CORRECTED RX PROCESSING ---
            # buff is already the complex CuPy array from the SDR (SOAPY_SDR_CF32 stream)
            s = buff.copy()
            
            # Perform FFT using CuPy
            S = cp.fft.fftshift(cp.fft.fft(s, N) / N)
            
            # Convert CuPy result to NumPy for Matplotlib
            S_np = cp.asnumpy(S) 
            
            # Frequency Domain Plot
            plt.subplot(212)
            # Frequencies calculated using NumPy
            ...
            print(f"An error occurred in the RX loop: {e}")
            tx_stop_event.set()
            break
