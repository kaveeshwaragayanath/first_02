...
def main(args):
    ...
    X, Y_complex_data = train_generator1.__getitem__(0)
    ...
    # To compare magnitude, we use the mean square magnitude (power)
    ...
    # Calculate the frequency index corresponding to pre_tx_samples duration
    noise_freq_end_index = int(pre_tx_samples * (N / total_rx_samples))
    if noise_freq_end_index < N:
        plt.axvline(x=f_ghz[noise_freq_end_index], color='g', linestyle=':', label='End of Pre-TX Noise (Time Index)')
